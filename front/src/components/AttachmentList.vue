<template>

    <div class="attach-wrapper">

        <div class="attach" v-for="(a,i) in attachments" :key="a.id">

          <div v-if="a.type==='loading'" class="attach-loading">
            <LoadingSvg fill="#222"/>
          </div>

          <a v-else :href="a.type==='image' ? base_url + '/storage/upload/' + a.name : a.name"  target="_blank" >

            <img v-if="a.type==='image'" :src="base_url + '/storage/upload/' + a.preview" :alt="a.alt">
            <img v-if="a.type==='tenor_gif'" :src="a.preview" :alt="a.alt">

            <div class="attach-delete" v-if="edited" @click="delAttach(i, $event)">
              <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path opacity="1" d="M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z"/>
                <path d="M13.0594 12.0001L15.3594 9.70011C15.6494 9.41011 15.6494 8.93011 15.3594 8.64011C15.0694 8.35011 14.5894 8.35011 14.2994 8.64011L11.9994 10.9401L9.69937 8.64011C9.40937 8.35011 8.92937 8.35011 8.63938 8.64011C8.34938 8.93011 8.34938 9.41011 8.63938 9.70011L10.9394 12.0001L8.63938 14.3001C8.34938 14.5901 8.34938 15.0701 8.63938 15.3601C8.78938 15.5101 8.97937 15.5801 9.16937 15.5801C9.35937 15.5801 9.54937 15.5101 9.69937 15.3601L11.9994 13.0601L14.2994 15.3601C14.4494 15.5101 14.6394 15.5801 14.8294 15.5801C15.0194 15.5801 15.2094 15.5101 15.3594 15.3601C15.6494 15.0701 15.6494 14.5901 15.3594 14.3001L13.0594 12.0001Z"
                      fill="#fff"/>
              </svg>
            </div>

          </a>
        </div>

    </div>


</template>

<script>
import {API_URL} from '@/api/config';
import LoadingSvg from "@/components/Loading";

export default {
  name: "AttachmentList",
  components: {LoadingSvg},
  props: {
    attachments: Object,
    edited: Boolean
  },
  data(){
    return {
      base_url: API_URL,
      modal: {
        show: false,
        url: "",
      }
    }
  },

  methods: {

    delAttach(i, e){

      e.preventDefault()

      this.$emit("onDelAttach", i)

    }
  }
}
</script>

<style scoped>

.attach .attach-loading svg {
  height: 24px;
  width: 24px;
}

.attach .attach-loading {
  display: flex;
  height: 80px;
  align-items: center;
  justify-content: center;
  margin: 0 10px 0 0;

  background-color: #fff;
  border-color: #bdd6fa;
  -webkit-box-shadow: 0 0 0 3px rgba(70, 131, 217, 0.4);
  box-shadow: 0 0 0 3px rgba(70, 131, 217, 0.4);

  padding: 0 20px;
  border-radius: 4px;
}

</style>